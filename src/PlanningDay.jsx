...
moment.locale('fr');
const START_HOUR = 11; const END_HOUR = 21;

// (Helper functions calculateEventLayout and WeekStripHeader remain the same)
const calculateEventLayout = (events) => { const sortedEvents = [...events].sort((a, b) => { if (a.start.getTime() !== b.start.getTime()) { return a.start.getTime() - b.start.getTime(); } return (b.end.getTime() - b.start.getTime()) - (a.end.getTime() - a.start.getTime()); }); const laidOutEvents = []; for (const event of sortedEvents) { let maxColumns = 1; let columnIndex = 0; for (const placedEvent of laidOutEvents) { const startsBeforePlacedEnds = event.start.getTime() < placedEvent.end.getTime(); const endsAfterPlacedStarts = event.end.getTime() > placedEvent.start.getTime(); if (startsBeforePlacedEnds && endsAfterPlacedStarts) { maxColumns = Math.max(maxColumns, placedEvent.layout.totalColumns + 1); } } const placedInColumns = new Array(maxColumns).fill(false); for (const placedEvent of laidOutEvents) { const startsBeforePlacedEnds = event.start.getTime() < placedEvent.end.getTime(); const endsAfterPlacedStarts = event.end.getTime() > placedEvent.start.getTime(); if (startsBeforePlacedEnds && endsAfterPlacedStarts) { placedInColumns[placedEvent.layout.columnIndex] = true; } } for (let i = 0; i < maxColumns; i++) { if (!placedInColumns[i]) { columnIndex = i; break; } } laidOutEvents.push({ ...event, layout: { totalColumns: maxColumns, columnIndex }}); } return laidOutEvents.map(event => { let newTotalColumns = event.layout.totalColumns; for (const otherEvent of laidOutEvents) { if (otherEvent.layout.columnIndex > event.layout.columnIndex) { const startsBeforeOtherEnds = event.start.getTime() < otherEvent.end.getTime(); const endsAfterOtherStarts = event.end.getTime() > otherEvent.start.getTime(); if (startsBeforeOtherEnds && endsAfterOtherStarts) { newTotalColumns = Math.max(newTotalColumns, otherEvent.layout.totalColumns); } } } return { ...event, layout: { ...event.layout, totalColumns: newTotalColumns }}; }); };
const WeekStripHeader = ({ currentDate, setDate }) => { const startOfWeek = moment(currentDate).startOf('week'); const days = Array.from({ length: 7 }, (_, i) => startOfWeek.clone().add(i, 'days')); return ( <div style={styles.headerContainer}> <div style={styles.weekStrip}> {days.map(day => ( <div key={day.format('YYYY-MM-DD')} style={styles.dayCell} onClick={() => setDate(day.toDate())}> <span style={styles.dayName}>{day.format('dd').charAt(0)}</span> <span style={{...styles.dayNumber, ...(moment(currentDate).isSame(day, 'day') && styles.activeDayNumber)}}>{day.format('D')}</span> </div> ))} </div> <div style={styles.fullDateDisplay}>{moment(currentDate).format('dddd — D MMM YYYY')}</div> </div> ); };
...
export default function PlanningDay({ events = [], resources = [], date, setDate, onEventMove, onEventClick, onGridClick }) {
    ...
    const [touchStartX, setTouchStartX] = useState(null); const [touchStartY, setTouchStartY] = useState(null);
    useEffect(() => { polyfill({ dragImageTranslateOverride: scrollBehaviourDragImageTranslateOverride }); const handleTouchMove = (e) => { if (document.querySelector(".dnd-poly-drag-image")) { e.preventDefault(); } }; window.addEventListener('touchmove', handleTouchMove, { passive: false }); return () => window.removeEventListener('touchmove', handleTouchMove); }, []);
    ...
    const resourceMap = new Map(safeResources.map(r => [r.id, r]));
    const laidOutEvents = useMemo(() => calculateEventLayout(safeEvents), [safeEvents]);
    const scrollContainerRef = useRef(null);

    // --- *** THIS IS THE FINAL FIX *** ---
    // The `eventOptions` object tells the gesture library that the events it's
    // listening to are NOT passive, giving it permission to prevent scrolling.
    useGesture({
        ...
        eventOptions: { passive: false } // This line solves the error
    });
